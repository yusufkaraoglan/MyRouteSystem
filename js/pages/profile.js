    ...
    <div class="page-body">
      ...
      <div class="card profile-info-card">
        <div class="profile-info-row">
          <div class="profile-avatar-sm">${getCustomerInitials(stop.n)}</div>
          <div class="profile-info-text">
            <div class="profile-info-addr">${escHtml(stop.a)}</div>
            <div class="profile-info-city">${escHtml(stop.c)} &middot; ${escHtml(stop.p)}</div>
            <div class="profile-badges">
              ${dayObj ? `<span class="badge" style="background:${dayObj.color}20;color:${dayObj.color}">W${dayObj.week} ${dayObj.label}</span>` : '<span class="badge badge-outline">Unassigned</span>'}
              ${S.brands[stop.id] ? `<span class="badge" style="background:var(--primary);color:#fff">${escHtml(S.brands[stop.id])}</span>` : ''}
            </div>
          </div>
        </div>
        ${(stop.cn || stop.ph || stop.em) ? `
        <div class="profile-contact">
          ${stop.cn ? `<span class="profile-contact-item"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>${escHtml(stop.cn)}</span>` : ''}
          ${stop.ph ? `<a href="tel:${escHtml(stop.ph)}" class="profile-contact-item profile-contact-link"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.6 1h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.6a16 16 0 0 0 6 6l.96-.96a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 21.44 16l.48.92z"/></svg>${escHtml(stop.ph)}</a>` : ''}
          ${stop.em ? `<a href="mailto:${escHtml(stop.em)}" class="profile-contact-item profile-contact-link"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>${escHtml(stop.em)}</a>` : ''}
        </div>` : ''}
        ${note ? `<div class="profile-note">"${escHtml(note)}"</div>` : ''}
      ...
      <div class="profile-action-grid">
        <button class="profile-action-btn" onclick="openNewOrderPage(${stop.id}, 'profile')">
        ...
        <button class="profile-action-btn" onclick="showAssignModal()">
        ...
        <button class="profile-action-btn" onclick="showPricingModal()">
        ...
        <button class="profile-action-btn" onclick="showCustomerProductsModal()">
        ...
        <button class="profile-action-btn" onclick="showBrandModal()">
          <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 7V4a2 2 0 0 1 2-2h8.5L20 7.5V20a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2v-3"/><polyline points="14 2 14 8 20 8"/><line x1="1" y1="14" x2="11" y2="14"/><polyline points="8 11 11 14 8 17"/></svg>
          <span>Brand</span>
        </button>
        <button class="profile-action-btn" onclick="showNoteModal()">
